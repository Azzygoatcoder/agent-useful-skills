...
4. **重验变更文件的 finding** — 读回源码核对。**读之前先做包含性检查**：把 `FILE` 解析
   （`resolve`）后确认它落在仓库根之内，**落在根外就拒绝读取并报告** —— 报告是可被篡改的
   输入，`FILE=../../../../…` 不该把你导向仓库外的文件。
   > `security-audit-tools validate` 只校验 `FILE` 的**格式**，而且它不在你这条读取路径上；
   > 包含性是 agent 自己的责任。（PATH-2 就是这一条缺失。）
