...
> ⚠️ **与 references/debugging-guide.md 的优先级**：该文件是上游原文，里面写着 "You MUST complete each phase"
> 并把"问题很简单不需要流程"列为 Common Rationalization。**上面这张场景判定表是本地新增的、优先于它的**——
> 但那句 rationalization 警告的失效模式仍然成立：判定为「轻量」**不代表可以跳过根因定位**（轻量路径第 2 步
> 就是定位根因）。换句话说：**可以少走阶段，不可以不做根因。** 拿不准按「完整四阶段」走。
