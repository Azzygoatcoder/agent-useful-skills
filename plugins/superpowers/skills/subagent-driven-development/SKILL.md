...
and fix-round diffs need it. Never use `HEAD~1` here: it silently drops
all but the last commit of a multi-commit task.
...
**DONE:** Generate the review package (`scripts/review-package PLAN_FILE BASE HEAD`, from this skill's directory — it prints the unique file path it wrote), then dispatch the task reviewer with the printed path. (BASE is the dispatch-time commit — never `HEAD~1`.)
...
- Hand the reviewer its diff as a file: run this skill's
  ...
  call. Use the dispatch-time BASE — never `HEAD~1`. Never dispatch a
