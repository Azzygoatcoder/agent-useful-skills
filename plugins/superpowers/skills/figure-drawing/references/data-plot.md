...
### D4. 渲染检查（必做）
- **原生读图**看图：图例冲突 / 未标注序列 / 坐标轴截断 / 文字溢出 / 配色
- 数值准确性由代码保证（值来自数据）；读图查**视觉错误**
- 需要独立判断时再用 `vision.py`（独立模型）复核同一张图——复核关注的是"我是不是看漏了"，
  而不是"帮我描述这张图"（后者原生读图就够）
- 与概念图共用验证环，但标准不同：数据图看"清楚 + 值对"，概念图看"设计一致"
