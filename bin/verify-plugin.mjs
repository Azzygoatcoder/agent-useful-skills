...

{
  ...
  const expectedVisible = dirs.filter((d) => !existing.has(d))
  check(
...
// --- 全新机器模拟：空 DSH_HOME ---
{
  ...
  try {
    ...
    check('全新机器 list() 返回全部技能', names.length === dirs.length && JSON.stringify(names) === JSON.stringify([...dirs].sort()), `${names.length}/${dirs.length}`)
