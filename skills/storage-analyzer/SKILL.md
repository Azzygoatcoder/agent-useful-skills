...
- **Windows**：**已实测（2026-08-12，Windows 11）**。`scan.py` 扫描跑通、UTF-8 输出修复（`sys.stdout.reconfigure`，否则中文目录名输出成 GBK 崩下游）；`/tmp` 路径改用 `$TEMP`。`build_report.py` 静态报告已验证。`server.py` 的回收站删除（`SHFileOperationW`）代码在但**未在 Windows 实测过删除动作**——首次真删前先在废纸篓验证。多盘符已支持（主盘分段条 + 其他盘列表）。
...
| 日期 | 学习来源 | 吸收的模式 |
|------|---------|-----------|
| 2026-08-12 | Windows 本地化改造 | **scan.py UTF-8 修复**（Windows stdout 默认 GBK，重定向文件中文目录名变 GBK → 下游 UTF-8 读崩；`sys.stdout.reconfigure`）；**`/tmp` 路径在 Windows 不适用**（Python/Read 不解析，改用 `$TEMP`）；实测 Windows 11 扫描 4 盘符 + build_report 静态报告通过；server.py 删除动作未实测（SHFileOperationW 待真删前废纸篓验证） |
